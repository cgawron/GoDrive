		...
		TreeNode, Comparable<Node>, MementoOriginator// , PropertyChangeListener
{
	...
	public class SiblingsIterator implements Iterator<Node>
	{
		...
		{
			while (parent != null && i < parent.children.size()
		...
		{
			return parent != null && i < parent.children.size();
		}
	...
	public void regionChanged(GobanEvent event)
	{
		...
		if (mm.getRegion() != null && oldView != newView) {
			...
			logger.info("regionChanged: region is null");
							...
							if (goban != null) {
								...
								if (n.isBeginOfVariation() || p.isDiagram()) {
									if (goban instanceof MarkupModel) {
										...
										logger.info("ResetMarkup not called, no MarkupModel");
								}
	...
	public void setGoban(Goban newGoban, boolean doMarkup)
	{
		...
		logger.info("Node " + this + ": Setting goban to " + newGoban.getClass());
	...
	public BoardType turn()
	{
		BoardType turn = BoardType.BLACK;

		Node node = this;
		while (!node.isMove() && node.parent != null)
			node = node.parent;

		if (node.isMove()) {
			if (node.contains(Property.BLACK))
				turn = BoardType.WHITE;
		}
